...
/*
The main difference between these two types of exceptions is in how they are handled by the Java compiler.

Checked exceptions are exceptions that must be either handled or declared in the method signature using the "throws" keyword. This means that if a method throws a checked exception, the caller must either handle the exception or declare that it throws the exception further up the call stack. Examples of checked exceptions in Java include IOException, SQLException, and ClassNotFoundException.

Unchecked exceptions, on the other hand, are exceptions that do not need to be declared in the method signature or handled explicitly in the code. The Java compiler does not require the programmer to handle or declare unchecked exceptions. Examples of unchecked exceptions in Java include NullPointerException, ArrayIndexOutOfBoundsException, and IllegalArgumentException.

In general, checked exceptions are used to handle exceptional conditions that are outside the control of the program, such as I/O errors or network problems, while unchecked exceptions are used to handle programming errors, such as null pointer references or array index out of bounds errors.

-------
throw - throws

In Java, "throw" and "throws" are keywords used to handle exceptions.

"throw" is used to manually throw an exception from within a method. When an exception is thrown using the "throw" keyword, the program execution stops immediately and the exception object is created and thrown back to the calling method or caught by a try-catch block.
